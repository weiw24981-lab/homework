...
func NewTestDB(config *model.Config) *gorm.DB {

	dbType := getDBType(config.DataTypeCon.DataType)
	var db *gorm.DB
	var err error
...
func newSQLiteDB(config *model.Config) (*gorm.DB, error) {
	...
	filename := config.SqliteCon.Filename
...
func newMySQLDB(config *model.Config) (*gorm.DB, error) {

	dsn := fmt.Sprintf("%s:%s@tcp(%s:%d)/%s?charset=utf8mb4&parseTime=True&loc=Local", config.MysqlCon.Username, config.MysqlCon.Password, config.MysqlCon.Host, config.MysqlCon.Port, config.MysqlCon.DBName)

	if dsn == "" {
		dsn = "root:password@tcp(localhost:3306)/testdb?charset=utf8mb4&parseTime=True&loc=Local"
		log.Fatalf("using default MySQL DSN, set TEST_MYSQL_DSN in .env file or environment variable to override")
	}
...
func newPostgresDB(config *model.Config) (*gorm.DB, error) {
	dsn := fmt.Sprintf("host=%s user=%s password=%s dbname=%s port=%d sslmode=disable TimeZone=Asia/Shanghai", config.ProgresCon.Host, config.ProgresCon.Username, config.ProgresCon.Password, config.ProgresCon.DBName, config.ProgresCon.Port)
	if dsn == "" {
		dsn = "host=localhost user=postgres password=password dbname=testdb port=5432 sslmode=disable TimeZone=Asia/Shanghai"
		log.Fatal("using default PostgreSQL DSN, set TEST_POSTGRES_DSN in .env file or environment variable to override")
	}
